const Table = ({ element, total }) => {
  ...
  return (
    <div className=" bg-white shadow-md rounded-md flex flex-col mt-10 mx-auto p-2 w-auto">
      <Content border={"border-green-500"}  name ={"Daily Total"} value={total}/>
...
export const Content = ({ name, value, border }) => {
  return (
    <span
      className={`flex justify-start content-between border-l-8 bg-zinc-100 h-24 my-4 ml-2 w-44 shad rounded-lg p-4 text-md font-bold text-zinc-600 ${border}`}
    >
